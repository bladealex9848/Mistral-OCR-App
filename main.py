...
if st.button("Process"):
    if source_type == "URL" and not input_url.strip():
        ...
        for idx, source in enumerate(sources):
            ...
            with st.spinner(
                f"Processing {source if source_type == 'URL' else source.name}..."
            ):
                try:
                    ...
                    time.sleep(
                        1
                    )  # wait 1 second between request to prevent rate limit exceeding

                    pages = (
                        ocr_response.pages
                        if hasattr(ocr_response, "pages")
                        else (ocr_response if isinstance(ocr_response, list) else [])
                    )
                    result_text = (
    ...
    for idx, result in enumerate(st.session_state["ocr_result"]):
        ...
        with col2:
            ...
            create_download_link(
                json_data, "application/json", f"Output_{idx+1}.json"
            )  # json output
            create_download_link(
                result, "text/plain", f"Output_{idx+1}.txt"
            )  # plain text output
            create_download_link(
                result, "text/markdown", f"Output_{idx+1}.md"
            )  # markdown output
